...
test('registration fails with invalid sleeper username', function () {
    ...
    // The custom validation rule handles the API validation
    $response = Volt::test('auth.register')
        ->set('name', 'Test User')
        ->set('email', 'test@example.com')
        ->set('password', 'password')
...
// Note: Unique validation is handled by the 'unique' rule in the validation array
// and doesn't need a separate test since it's already covered by Laravel's built-in validation
