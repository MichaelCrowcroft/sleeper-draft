...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('player_stats', function (Blueprint $table) {
            ...
            $table->string('opponent', 10)->nullable();
            $table->string('game_id')->nullable();
            $table->string('company')->nullable();
            $table->string('category')->default('stat');
            ...
            $table->unique(['sleeper_player_id', 'season', 'week', 'season_type'], 'unique_player_week_stats');
        });
    }
    ...
    {
        Schema::table('player_stats', function (Blueprint $table) {
            $table->dropUnique('unique_player_week_stats');
            $table->dropIndex(['sleeper_player_id', 'season', 'week']);
            $table->dropIndex(['season', 'week']);
            $table->dropIndex(['game_date']);
            $table->dropForeign(['player_id']);
            $table->dropColumn([
                'player_id', 'sleeper_player_id', 'season', 'week', 'season_type', 'game_date',
                'team', 'opponent', 'game_id', 'company', 'category', 'status',
